...
class COCO(DETECTION):
    def __init__(self, db_config, split=None, sys_config=None):
        ...
        # self._coco_cls_ids = [i for i in range(0, 11)]
        self._coco_cls_ids = [i for i in range(0, 13)]
        ...
        self._cls2coco = {
            ind + 1: coco_id for ind, coco_id in enumerate(self._coco_cls_ids)
        }
        self._coco2cls = {coco_id: cls_id for cls_id, coco_id in self._cls2coco.items()}
        self._coco2name = {
            cls_id: cls_name
            for cls_id, cls_name in zip(self._coco_cls_ids, self._coco_cls_names)
        }
        self._name2coco = {
            cls_name: cls_id for cls_name, cls_id in self._coco2name.items()
        }

        if split is not None:
            # coco_dir = os.path.join(sys_config.data_dir, "coco")
            coco_dir = os.path.join(sys_config.data_dir)
            ...
            #     coco_dir, "annotations", "instances_{}.json".format(self._split)
            # )
            self._data_dir = os.path.join(coco_dir, self._split, "images")
            self._anno_file = os.path.join(coco_dir, self._split, "annotation.json")
