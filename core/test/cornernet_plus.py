...
def _crop_image(image, center, size, output_size=None):
    if output_size == None:
        output_size = size
    ...
    cropped_image       = np.zeros((o_height, o_width, 1), dtype=image.dtype)
...
def cornernet_inference(db, nnet, image, image_mask, decode_func=decode):
    K             = db.configs["top_k"]
    ...
    for scale in scales:
        ...
        images = np.zeros((1, 4, inp_height, inp_width), dtype=np.float32)
        ...
        resized_image  = torch.from_numpy(resized_image).cuda()
        resized_image = resized_image.unsqueeze(0).float()
        resized_image -= im_mean
        resized_image /= im_std
        images[0, 0:3] = resized_image.squeeze(0).cpu().numpy()
        
        resized_image_mask = cv2.resize(image_mask, (new_width, new_height))
        resized_image_mask = np.expand_dims(resized_image_mask, axis=-1)
        resized_image_mask, border, offset = _crop_image(resized_image_mask, new_center, [inp_height, inp_width])
        resized_image_mask = resized_image_mask.astype(np.float32) / 255.
        images[0, -1] = resized_image_mask.transpose((2, 0, 1))
